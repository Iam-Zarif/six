...
let loadData1 = (data) => {
  ...
  for (let i of data) {
    ...
    <div><h3>${i.name}</h3>
    <h5 class="text-secondary"><img src="./media/timetable.png" alt="date">  ${i.published_in}</h5></div>
  ...
  <div class="modal-dialog">
    <div class="modal-content p-lg-5" >
      ...
      <div class="modal-header">
        ...
        <button type="button" class="bg-danger p-3 rounded-5 btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body" id= "modal-body">
         
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-danger p-3 rounded-5" data-bs-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
...
let popUpDetails = (data) => {
  ...
  <div class="card-body container">
    ...
    <div class= "bg-warning-subtle px-lg-3 py-lg-2 rounded-4">
...
<div class ="container"><h3 class="mt-4">Features</h3>
   <li class="card-text fw-semibold mt-4 text-secondary">  ${
     data.features[1].feature_name
   }</li>
        <li class="card-text fw-semibold text-secondary">  ${
...
<li id="integration1" class=" card-text fw-semibold mt-4 text-secondary">${
    data.integrations[0]
  }</li>
        <li id="integration2" class=" card-text fw-semibold text-secondary">${
          data.integrations[1]
        }</li>
        <li id="integration3" class=" card-text fw-semibold text-secondary">${
          data.integrations[2]
        }</li>
...
<div class ="card container">
   ...
   <button id="accuracy" type="" class="btn btn-danger  pt-2">${
     data.accuracy.score * 100
   }</button><span class="text-white fw-bold py-2 px-2 rounded-3 mt-5 bg-danger">% accuracy</span> 
   </section>
   <img src ="${data.image_link[0]}" class ="img-fluid p-4"></div>
   <h3 class = "text-center">${data.input_output_examples[0].input}</h3>
   <h5 class = "text-center mt-4 p-lg-4 text-secondary">${
  ...
  // console.log(integration1.innerText);
  // console.log(integration2.innerText);
  // console.log(integration3.innerText);
...
accuracySection.innerHTML='';
        ...
        accuracySection.innerHTML = `<button id="accuracy" type="" class="btn btn-danger  pt-2">${
     data.accuracy.score * 100
   }</button><span class="text-white fw-bold py-2 px-2 rounded-3 mt-5 bg-danger">% accuracy</span> `
  }
